...
# =======================================================
PAPER_TRADING = True  # UBAH KE False JIKA SUDAH SIAP EXEKUSI DI AKUN REAL REAL MONEY!
...
with st.sidebar:
    ...
    st.subheader("📝 Perintah Strategi AI (Locked in Code for Efficiency)")
...
def hitung_manajemen_risiko(keputusan, harga_masuk, atr_value):
    ...
    try:
        KURS_USD_IDR = requests.get("https://er-api.com", timeout=3).json()["rates"]["IDR"]
    except Exception:
        KURS_USD_IDR = 16000
        
    RISIKO_USD = 50000 / KURS_USD_IDR
    LOT_MIN = 0.01
    LOT_MAX = 0.05  # Kunci Safety Lot Akun 1 Juta Anda
    ...
    return {"action": keputusan, "price": harga_masuk, "lot": lot_eksekusi, "sl": round(stop_loss, 5), "tp": round(take_profit, 5)}
...
def eksekusi_order_mt5(order_plan, symbol_name):
    ...
    if PAPER_TRADING:
        return f"🚨 [MODE DEMO ACTIVE] Rencana order berhasil dibuat: {order_plan['action']} {order_plan['lot']} Lot pada harga {order_plan['price']} (SL: {order_plan['sl']}, TP: {order_plan['tp']})"
...
st.caption("Sistem Robot Otomatis: Menarik Data Live Broker ➔ Analisis MTF ➔ Auto-Entry Real Account.")
...
if st.button("🚀 Aktifkan Siklus Auto-Analisis & Entry Sekarang"):
    ...
    # 1. Mengaktifkan Koneksi ke Aplikasi MetaTrader 5 di Server/Komputer
    ...
    # 2. Ambil Data Harga Live Multi-Timeframe dari Broker (Tanpa Perlu File CSV Lagi!)
    ...
    if rates_h1 is None or rates_m15 is None:
        st.error(f"Gagal menarik data harga untuk symbol {symbol}. Pastikan nama pasarnya sudah benar sesuai jenis di broker Anda!")
        mt5.shutdown()
        st.stop()
    ...
    # 3. Kirim Perintah Analisis ke Otak AI OpenAI
    try:
        client = OpenAI(api_key=api_key)
        harga_sekarang = df_m15['close'].iloc[-1]
        atr_terakhir = df_m15['atr'].iloc[-1]
        
        prompt_system = f"""You are Trading Intelligence Analyst V3 Akun Real Modal Rp1 Juta.
Aturan: 
1. Baca Tren Makro H1 via EMA. Jika Sideways wajib NO TRADE.
2. Jika H1 searah, cari konfirmasi di M15 menggunakan BOS, ICT, dan Fibonacci (61.8%-78.6%).
3. Entry minimal didukung 2 dari 3 konfirmasi di M15. Jika hanya 1 konfirmasi, wajib NO TRADE.
4. Berikan hasil akhir keputusan di baris paling bawah dengan format teks baku: STRATEGY_DECISION: BUY atau STRATEGY_DECISION: SELL atau STRATEGY_DECISION: NO TRADE."""
        ...
        # 4. Filter Otomatis Keputusan AI & Hitung Lot Pengaman
        keputusan = "NO TRADE"
        ...
        if rencana_keuangan:
            ...
            # Menembak posisi order otomatis langsung ke platform MT5 Anda!
        ...
        mt5.shutdown() # Memutus koneksi dengan aman setelah tugas selesai
